...
External Debate/Reflexion and Exp-C remain unrun at the time of writing.
...
status-prior tables are retained below only as preliminary context. Under
thin replication ($n{=}3$ per cell) we restrict ourselves to
\emph{directional} language---consistent sign and ordering patterns across
cells---and reserve confirmatory claims for the expanded grid; SSR is never
interpreted in isolation from attribution (Section~\ref{sec:exp_metrics}).
...
rather than silently mixed: the measured hybrid tables below predate a
later hardening of the data layer, a deterministic catalog and validator
constraining the data-access guide used in forward and backward passes.
Because that hardening touches the same paths the diagnosis loop
exercises, post-hardening replications are not assumed poolable with the
current cells: the expanded grid re-establishes its tables under the
hardened implementation, preceded by a no-plant end-to-end gate confirming
that the un-faulted pipeline still terminates in \emph{Practical Optimal};
each table carries the campaign, and hence the implementation freeze, it
was produced under.
...
\paragraph{Measured results (evidence-informed; thin $n{=}3$).}
Table~\ref{tab:exp_ablation} reports the redesigned ablation on
\texttt{smoke\_H4\_Omega5} with plant \texttt{me\_force\_zero\_sea},
$K{=}3$, provider DashScope / \texttt{deepseek-v4-flash}, and
\texttt{rank\_method}=\texttt{hybrid} (force-zero smell tips
\texttt{model\_expert} without inventing a second diagnosis mechanism).
Causal commitment achieves first-probe $3/3$ and the only non-zero verified
attribution ($1/3$); reverse and random are $0/3$ on both primary metrics.
SSR@$K$ likewise concentrates on causal ($2/3$). These counts are
directional under thin $n$; they are not a definitive Proposition~1 proof.

\begin{table}[t]
\centering
\caption{Exp-A commitment-order ablation --- \textbf{measured}
(evidence-informed; $n{=}3$/order; $K{=}3$; hybrid rank). Primary: first-probe
hit and verified attribution.}
\label{tab:exp_ablation}
\label{tab:exp_i}
\begin{tabular}{@{}l c c c c@{}}
\toprule
$\omega$ & First-probe hit & Verified attr.\ & SSR@$K$ & Mean tokens \\
\midrule
causal  & $3/3$ (1.00) & $1/3$ (0.33) & $2/3$ & $1.08{\times}10^{5}$ \\
random  & $0/3$ (0.00) & $0/3$ (0.00) & $0/3$ & $1.26{\times}10^{5}$ \\
reverse & $0/3$ (0.00) & $0/3$ (0.00) & $0/3$ & $1.15{\times}10^{5}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Measured results (evidence-informed; internal only; thin $n{=}3$).}
Table~\ref{tab:exp_baselines} reports the redesigned internal comparison on
the same plant, model, and $K$ as Exp-A, using the Exp-A causal cells as the
Stackelberg arm. Verified attribution is $1/3$ for causal Stackelberg and
$0/3$ for both adversarial and sequential (gate: SB $\geq$ adversarial).
First-probe remains $3/3$ under Stackelberg versus $1/3$ adversarial.
SSR@$K$ is highest for Stackelberg ($2/3$) on this draw; sequential's mean
token count is depressed by one zero-token failed episode
(\texttt{eb\_seq\_r1}) and should be read with that caveat. Debate and
Reflexion were not run in this pass (cost freeze until SB verified rates
stabilize).

\begin{table}[t]
\centering
\caption{Exp-B internal baselines --- \textbf{measured}
(evidence-informed; $n{=}3$; $K{=}3$). Primary: verified attribution.
External Debate/Reflexion not run.}
\label{tab:exp_baselines}
\label{tab:exp_ii}
\label{tab:exp_iii}
\begin{tabular}{@{}ll c c c c@{}}
\toprule
Family & Method & Verified attr.\ & First-probe & SSR@$K$ & Mean tokens \\
\midrule
Proposed & stackelberg (causal)
  & $1/3$ (0.33) & $3/3$ (1.00) & $2/3$ & $1.08{\times}10^{5}$ \\
Internal & adversarial
  & $0/3$ (0.00) & $1/3$ (0.33) & $1/3$ & $1.84{\times}10^{5}$ \\
Internal & sequential
  & $0/3$ (0.00) & $0/2$ (0.00) & $1/3$ & $0.61{\times}10^{5}$ \\
External & Debate / Reflexion
  & --- & --- & --- & not run \\
\bottomrule
\end{tabular}
\end{table}
