...
The measured mechanism is precise: alignment of the committed order with the
guilty layer fully determines first-probe attribution (zero exceptions over
every random arm), causal commitment dominates single-judge, debate, and
reflexion naming ($8{:}0$ discordant pairs) at the lowest token cost, the
guilty layer never deflects under commitment, and $46$--$71\%$ of complying
...
\item Alignment of the committed order with the guilty layer fully determines first-probe attribution --- zero exceptions across all random arms.
\item Committed inspection dominates single-judge, debate, and reflexion naming at the lowest token cost; guilty layers never deflect under commitment.
\end{highlights}
...
three problem instances, five diagnosis protocols, and one hundred and
forty reconstructable probe rounds, is precise about the mechanism:
\emph{committed alignment carries the information}. First-probe
attribution occurs exactly when the committed $\omega_1$ equals the
guilty layer --- with zero exceptions over every random arm ever run ---
and the ranking that feeds the commitment sees the model layer (through
...
every fixed order. The order-effect claim is therefore conditionally
...
shared frozen blackboards of the main grid, causal Stackelberg attains
first-probe attribution on every board and dominates single-judge,
debate, and reflexion naming ($8{:}0$ discordant pairs each); verified
attribution reaches $0.60$ versus $0.20$ under reverse commitment on the
instance family (pooled paired $p=0.008$); and the budget diagnostics
show that within $K{=}3$ nothing improves after the second round ---
the guilty layer's position, not budget depth, is what the curves
respond to. External multi-call protocols do not overturn the design:
debate and reflexion name no better than one judge, and reflexion's
non-significant verified edge ($0.55$ versus $0.45$) costs $2.8\times$
the loop tokens without a committed order or its audit trail. Two
mechanism checks close the loop between theory and logs: the guilty
layer never deflected under commitment with executed refutation
(truthful best response as designed), while $46$--$71\%$ of complying
repairs were overturned by the re-solve --- verification, not cheap
talk, is what carries attribution.

The claims are bounded accordingly: the truthful-deflect result is
established on the model-layer plant (whose guilt is visible in its own
output), the instance family varies horizon and scenario count at fixed
network size, all campaigns run a single engine under a frozen
implementation, and $K$ is capped at three. Within those bounds the
...
semantic data layer, scaling the network, and testing cross-engine
robustness of the ranking are the registered next steps.
