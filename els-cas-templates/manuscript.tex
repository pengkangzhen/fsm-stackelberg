...
reflexion naming (paired across all twenty shared boards without a single
...
Figure~\ref{fig:expa} summarizes the ablation and the alignment audit.
...
snapshot mode; $n{=}20$/order; plant \texttt{me\_force\_zero\_sea}).
(a) First-probe hit, verified attribution, and SSR@$K$ by probe order.
(b) Seed-level alignment audit of the random arm: a first-probe hit
occurs if and only if the committed shuffle places the guilty layer first
($\omega_1=a^{\star}$; $10/10$ with zero exceptions) --- the random arm's
naming performance is carried entirely by accidental alignment with the
ranking. Seeds 7, 12, 14 had no completed run manifest and are outside
the campaign's $n{=}20$.}
\label{fig:expa}
\end{figure}
...
($0.55$, Wilson 95\% CI $[0.34, 0.74]$): a point estimate above
Stackelberg's $0.45$ (95\% CI $[0.26, 0.66]$), where the two intervals
overlap over most of their range and the paired difference is not
significant ($0{:}2$ discordant pairs, $p=0.5$), bought at
...
advantage. Figure~\ref{fig:expb} shows the accuracy--cost trade-off.
...
Exp-B methods ($n{=}20$ shared frozen blackboards each; error bars:
Wilson 95\% CIs). Stackelberg sits on the efficient frontier at the
lowest token cost; adversarial matches its point estimate only at
$1.9\times$ the cost, and the external multi-call protocols cost
$2.8\times$ (Reflexion) to $3.7\times$ (Debate). Reflexion's higher
point estimate ($0.55$ vs $0.45$) is not significant --- its interval
overlaps Stackelberg's over most of its range ($0{:}2$ discordant pairs,
$p=0.5$).}
\label{fig:expb}
\end{figure}
...
($6$ aligned hits at $K{=}1$, $8$ by $K{=}2$, $9$ at $K{=}3$). The
...
alignment of $\omega_1$ with $a^{\star}$, not budget depth (a $K{>}3$ sweep
was not run and is not claimed). Second, deflects are truthful: across roughly one
...
Figure~\ref{fig:expc} plots the step curves.
...
offline re-analysis; $n{=}20$ shared blackboards per method; cumulative
...
$\omega$: causal commitment saturates at $K{=}1$ ($9/20$), reverse reaches
its plateau only at $K{\ge}2$, sequential never leaves zero, and
Reflexion's $7\to11$ jump between $K{=}1$ and $K{=}2$ reflects
